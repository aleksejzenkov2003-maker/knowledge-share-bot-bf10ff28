...
export const PII_PATTERNS: PiiPatternConfig[] = [
  ...
  
  {
    ...
    patterns: [
      // Дата рождения ТОЛЬКО в контексте ключевых слов
  ...
  
  {
    ...
    patterns: [
      ...
      // Индекс + адрес (строго: 6 цифр + запятая + город)
      /\b\d{6}\s*,\s*(?:г\.?\s*)?[А-ЯЁа-яё]+/g,
      // Адрес проживания/регистрации с контекстом (не "в адрес")
      /(?:адрес\s*(?:проживания|регистрации|места\s*жительства|прописки))[:\s]*[^,\n]{10,80}/gi,
    ],
    priority: 25,
    enabled: true,
  ...
  
  {
    ...
    patterns: [
      ...
      // Имя + Фамилия без отчества — УДАЛЁН: слишком много ложных срабатываний на юридических терминах
